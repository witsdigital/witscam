...
<script type="text/javascript">
    ...
    function deleta() {
        window.location.href = '<?= base_url('publicar/apagar') ?>' + '/' + document.getElementById('id').value;
    ...
    <div class="box-body">
        ...
        <div class="box">
            <div class="box-header">
                <div align="right"><input  onclick="window.location.href = '<?= base_url() ?>publicar/cadastro'" type="button" value="Nova Publicação" class="btn btn-success"></button></div>
                        ...
                        foreach ($publicacoes as $row) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $row->entidade ?></td>
                                    ...
                                    ?><button onclick="window.location.href='<?=  base_url('publicar/status')?>/<?php echo $row->id ?>'" <?php echo $look?> class="btn btn-sm btn-<?php echo $classe?>"><?php echo $status?></button></td>
                                <td align="center" style="width: 20%;"><button onclick="window.location.href = '<?= base_url('publicar/view_publicacao') ?>/<?php echo $row->id ?>'" class="btn btn-primary btn-flat">ver</button><button onclick="window.location.href = '<?= base_url('publicar/alterar') ?>/<?php echo $row->id ?>'" class="btn btn-warning btn-flat">Alterar</button><button onclick="confirma(<?php echo $row->id ?>);" class="btn confirma  btn-danger btn-flat" >Apagar</button></td>
